...
const myIcon = L.icon({
  iconUrl:
    "https://iclient.supermap.io/web/libs/leaflet/1.7.1/images/marker-icon-2x.png",
  iconSize: [25, 35]
});
...
async function ponit(points) {
  ...
  return await new Promise((resolve, reject) => {
    ...
    points.forEach(item => {
      const marker = L.marker(item.location, { icon: myIcon }).bindPopup(
  ...
  L.supermap
    ...
    .getThemeInfo(themeParameters, serviceResult => {
      serviceResult.type === "processFailed"
        ? console.log(serviceResult.error.errorMsg)
        : true;
...
async function recentData(reverseDate) {
  const recent_echartsData = await new Promise(resolve => {
    const sevenAgoTime = recentDate(7);
    const recent_echartsData = {
      data: [],
      time: [],
      count: []
    };
    sevenAgoTime.map(item => {
      ...
      reverseDate.map((item, index) => {
        const earthquakeTime = item.CATA_ID.slice(6, 10);
